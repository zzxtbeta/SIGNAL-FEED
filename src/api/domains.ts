...
function adaptDomainToTechNode(domain: DomainNode, parentId?: string): TechNode {
  ...
  // 根据paper_count推断trend
  let trend: 'rising' | 'stable' | 'declining' | 'early' = 'stable';
  if (domain.paper_count > 100) trend = 'rising';
  else if (domain.paper_count > 50) trend = 'stable';
  else if (domain.paper_count > 0) trend = 'early';
  else trend = 'early';
  
  // 根据level和paper_count推断stage
  let stage = '理论研究';
  if (domain.level === 'technology') {
    ...
    if (domain.paper_count > 100) stage = '工程化早期';
    else stage = '实验室阶段';
  } else {
    // domain level
    stage = '实验室阶段';
...
export const domainApi = {
  ...
  getDomainTree: async (): Promise<{ domains: TechNode[] }> => {
    try {
      ...
      console.error('❌ [v2] Error in getDomainTree:', error);
      return { domains: [] };
    }
  },
