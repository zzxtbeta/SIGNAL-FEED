    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm">
      <div className="bg-neutral-900 border border-neutral-800 rounded-lg w-full max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="sticky top-0 bg-neutral-900 border-b border-neutral-800 p-6 flex items-start justify-between">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              <span className="px-2 py-1 bg-orange-600/20 text-orange-600 text-xs font-bold rounded border border-orange-600/30">
                笔记
              </span>
        ...
        <div className="p-6 space-y-6">
          ...
          {note.tags.length > 0 && (
            <div>
              <h3 className="font-semibold text-sm text-neutral-400 mb-3 flex items-center gap-2">
              ...
              <div className="flex flex-wrap gap-2">
                {note.tags.filter(tag => tag).map((tag, idx) => (
                  <span
                    key={idx}
                    className="px-3 py-1.5 bg-neutral-800 text-neutral-300 text-sm rounded-lg border border-neutral-700"
                  >
                    {tag}
                  </span>
                ))}
          ...
          <div>
            ...
            <div className="bg-neutral-800/50 rounded-lg p-6 border border-neutral-700">
              <div className="prose prose-invert max-w-none">
                {note.content.split('\n').map((paragraph, idx) => (
                  <p key={idx} className="text-neutral-300 leading-relaxed mb-3 last:mb-0">
                    {paragraph || '\u00A0'}
                  </p>
                ))}
              </div>
            ...
            <div>
              ...
              <div className="bg-neutral-800 rounded-lg p-4">
                <div className="flex items-center justify-between">
                  ...
                  <button className="text-orange-600 hover:text-orange-500 text-sm font-semibold transition-colors">
                    查看关联信号 →
                  </button>
                </div>
              </div>
          ...
          <div className="bg-neutral-800/30 rounded-lg p-4 border border-neutral-700/50">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                ...
                <p className="text-neutral-200 mt-1">{note.createdAt}</p>
              </div>
              <div>
                <span className="text-neutral-500">最后更新</span>
                <p className="text-neutral-200 mt-1">{note.updatedAt}</p>
              </div>
              <div>
                <span className="text-neutral-500">笔记ID</span>
                <p className="text-neutral-200 mt-1 font-mono text-xs">{note.id}</p>
              </div>
              <div>
                <span className="text-neutral-500">关联信号数</span>
                <p className="text-neutral-200 mt-1">{note.relatedSignals} 条</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-3 pt-4 border-t border-neutral-800">
            ...
            <button
              ...
              className="flex-1 flex items-center justify-center gap-2 px-4 py-3 bg-neutral-800 hover:bg-red-600/20 hover:text-red-500 hover:border-red-600/50 rounded font-semibold transition-all border border-neutral-700"
            >
